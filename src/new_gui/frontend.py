...
class AutocorrectionPage(BasePage):
    def __init__(self, parent, backend) -> None:
        ...
        self.corrected_code_desc.grid(row=1, column=0, padx=20, pady=(5, 0), sticky="w")

        self.corrected_code_path = ctk.CTkEntry(self, placeholder_text=str(CORRECTED_CODE_DIR))
    ...
    def correct_command(self) -> None:
        try:
            ...
            self.backend.corrected_dir = Path(corrected_dir)

            print(processed_dir)
            print(self.backend.corrected_dir)
        finally:
